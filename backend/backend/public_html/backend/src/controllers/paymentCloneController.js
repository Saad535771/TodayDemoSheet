...
function applyPaymentCycleFields(payload = {}, beforeData = null) {
  ...
  // Previous month logic for cancelled tuitions
  const prevDate = new Date();
  prevDate.setMonth(prevDate.getMonth() - 1);
  const prevCycleDate = getPakistanMonthStartDateString(prevDate);
  ...
  if (nextCancelled) {
    // If it's newly cancelled or staying cancelled, move/keep it in the previous month cycle
    payload.paymentDate = prevCycleDate;
    payload.date = prevCycleDate;
    return payload;
  }

  // Active tuitions stay in current cycle
...
export function makePaymentCloneController({
  ...
  try {
    ...
    // Month/year filter intentionally removed.
    // API will send all payments_clone records.
    // Frontend will handle month filter, current month, previous month,
    // cancelled badge, numbering, and pagination.

    if (search) {
      const searchPattern = `%${search}%`;

      where[Op.or] = [
        { tuitionId: { [Op.like]: searchPattern } },
        { tuitionName: { [Op.like]: searchPattern } },
        { tutorName: { [Op.like]: searchPattern } },
        { subjects: { [Op.like]: searchPattern } },
        { className: { [Op.like]: searchPattern } },
        { country: { [Op.like]: searchPattern } },
        { otmName: { [Op.like]: searchPattern } },
        { status: { [Op.like]: searchPattern } },
        { feedback: { [Op.like]: searchPattern } },
        { notes: { [Op.like]: searchPattern } },
      ];
    }

    console.log(
      `[PAYMENT-LIST] Fetching ALL DATA | Page: ${page}, Limit: ${limit}, Month Param: ${month}, Year Param: ${year}, Search: ${search || "none"}`
    );
    console.log(`[PAYMENT-LIST] WHERE clause:`, JSON.stringify(where));

    const { count, rows } = await PaymentClone.findAndCountAll({
      where,
      order: [
        ["orderIndex", "ASC"],
        ["updated_at", "DESC"],
    ...
    console.log(`[PAYMENT-LIST] Sent ${rows.length} rows (Total: ${count})`);
    ...
    return res.json({
      ...
      currentPage: page,
      pageSize: limit,
      filtersHandledOnFrontend: true,
      receivedMonthParam: month,
      receivedYearParam: year,
    });
  } catch (err) {
