...
export function makePaymentCloneTeamBController({
  PaymentCloneTeamB,
  ...
  async function createAuditLog({
    ...
    try {
      await PaymentChangeRequest.create({
        ...
        notificationType: "payment_sheet_clone_team_b", // CHANGED HERE
      ...
      console.error("PAYMENT AUDIT LOG ERROR TEAM B:", err);
    }
  }
  ...
  function buildAuditMetadata({
    ...
    return {
      ...
      message,
      historyRowId: rowId,
      originalPaymentCloneTeamBId: rowId,
  ...
  return {
    ...
    async list(req, res) {
      try {
        ...
        console.error("PAYMENT CLONE TEAM B LIST ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error fetching payment clone team b rows",
        });
      }
    },
    ...
    async create(req, res) {
      try {
        ...
        if (orderIndex === undefined || orderIndex === null || orderIndex === "") {
          const lastRow = await PaymentCloneTeamB.findOne({
        ...
        const item = await PaymentCloneTeamB.create(payload);
        const afterData = serializeRow(item);

        await createAuditLog({
          req,
          actionType: "create",
          paymentCloneTeamBId: item.id,
          ...
          metadata: buildAuditMetadata({
            ...
            beforeData: null,
            afterData,
            message: "Row created directly in payment sheet team b",
          }),
        });

        return res.json({ item: afterData });
      } catch (err) {
        console.error("PAYMENT CLONE TEAM B CREATE ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error creating payment clone row team b",
        });
      }
    },
    ...
    async update(req, res) {
      try {
        const { id } = req.params;
        const row = await PaymentCloneTeamB.findByPk(id);

        if (!row) {
          return res.status(404).json({ message: "Payment clone team b row not found" });
        }
        ...
        try {
            ...
            if (changes.length > 0) {
                ...
                await PaymentCloneTeamB.sequelize.query(
                    `INSERT INTO payment_clone_team_b_histories (payment_clone_team_b_id, user_id, action_type, field_name, old_value, new_value) VALUES ${placeholders}`,
                    { replacements: flatValues }
                );
            }
        } catch (historyErr) {
            console.error("PAYMENT CLONE TEAM B HISTORY SAVE ERROR:", historyErr);
        }
        if (changedColumns.length) {
          await createAuditLog({
            req,
            actionType: "update",
            paymentCloneTeamBId: row.id,
            changedColumns,
            beforeData,
            afterData,
            metadata: buildAuditMetadata({
              ...
              beforeData,
              afterData,
              message: "Row updated directly in payment sheet team b",
            }),
          });
        }
        ...
        console.error("PAYMENT CLONE TEAM B UPDATE ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error updating payment clone team b row",
        });
      }
    },
    ...
    async remove(req, res) {
      ...
      try {
        ...
        const moved = await movePaymentCloneTeamBWithDateToTrash({
          PaymentCloneTeamB,
          PaymentCloneTrashTeamB,
          id,
          transaction,
        ...
        await createAuditLog({
          ...
          metadata: buildAuditMetadata({
            ...
            beforeData,
            afterData: null,
            message: "Row deleted from payment sheet team b and moved to trash",
          }),
        });

        return res.json({
          success: true,
          message: "Payment clone team b row moved to trash",
        });
      } catch (err) {
        ...
        console.error("PAYMENT CLONE TEAM B DELETE ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error deleting payment clone team b row",
        });
      }
    },
    ...
    async reorder(req, res) {
      try {
        ...
        const currentRows = await PaymentCloneTeamB.findAll({
        ...
        for (const entry of items) {
          ...
          await PaymentCloneTeamB.update(
            { orderIndex: Number(entry.orderIndex) || 0 },
            { where: { id } }
          );
        }

        const updatedRows = await PaymentCloneTeamB.findAll({
          attributes: ["id", "orderIndex"],
          order: [["orderIndex", "ASC"]],
        });
        ...
        if (!valuesAreSame(beforeOrder, afterOrder)) {
          await createAuditLog({
            ...
            metadata: {
              ...
              beforeOrder,
              afterOrder,
              message: "Rows reordered in payment sheet team b",
            },
          });
        }
        ...
        console.error("PAYMENT CLONE TEAM B REORDER ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error reordering payment clone team b rows",
        });
      }
    },
    ...
    async listTrash(req, res) {
      try {
        const rows = await PaymentCloneTrashTeamB.findAll({
          order: [["updated_at", "DESC"]],
        });

        return res.json({ items: rows.map(serializeRow) });
      } catch (err) {
        console.error("PAYMENT CLONE TEAM B TRASH LIST ERROR:", err);
        return res.status(500).json({
          message: err?.message || "Error fetching payment clone team b trash rows",
        });
      }
    },
    ...
    async restoreTrash(req, res) {
      ...
      try {
        ...
        const trashRow = await PaymentCloneTrashTeamB.findByPk(id, { transaction });
        if (!trashRow) {
          await transaction.rollback();
        ...
        const restored = await PaymentCloneTeamB.create(payload, { transaction });
        ...
        await createAuditLog({
          req,
          actionType: "create",
          paymentCloneTeamBId: restored.id,
          changedColumns: getChangedColumns(null, afterData, Object.keys(payload)),
          beforeData: null,
          afterData,
          metadata: {
            ...buildAuditMetadata({
              ...
              beforeData: null,
              afterData,
              message: "Row restored from trash to payment sheet team b",
            }),
        ...
        console.error("PAYMENT CLONE TEAM B RESTORE ERROR:", err);
        return res.status(500).json({
...
// --- NEW ADMIN HISTORY FETCH API (TEAM B) ---
    async getPaymentHistory(req, res) {
      try {
        ...
        let query = `
          ...
          FROM payment_clone_team_b_histories ph
        ...
        if (rowId) {
          query += ` AND ph.payment_clone_team_b_id = ?`;
        ...
        const historyData = await PaymentCloneTeamB.sequelize.query(query, {
          replacements,
          type: "SELECT"
        });
        ...
        console.error("Fetch Payment History Error Team B:", error);
    ...
    async forceDeleteTrash(req, res) {
      try {
        const { id } = req.params;
        const row = await PaymentCloneTrashTeamB.findByPk(id);
        ...
        console.error("PAYMENT CLONE TEAM B FORCE DELETE TRASH ERROR:", err);
        return res.status(500).json({
