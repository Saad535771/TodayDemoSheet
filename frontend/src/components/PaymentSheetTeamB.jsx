...
export default function PaymentSheetTeamB({ me, isActive = true, onCountChange }) {
 ...
 const canSeeTutorShare = isAdminRole || resolveAccessFlag(me, "access_tutor_share_clone_team_b");
  ...
  const applyUpdateEntries = async (updates, { historyLabel = "Edit", recordHistory = true } = {}) => {
    ...
    try {
     await Promise.all(normalized.map((entry) => api.patch(`/payments-clone-team-b/${encodeURIComponent(entry.rowId)}`, entry.afterPatch)));
      if (recordHistory) {
  ...
  const applyReorderChange = async (nextRows, historyLabel = "Reorder Rows") => {
    ...
    try {
    await api.post("/payments-clone-team-b/reorder", { items: afterOrder });
      rememberHistoryEntry({
        type: "reorder",
        label: historyLabel,
  ...
  const undoLastAction = async () => {
    ...
    try {
      if (entry.type === "updateMany") {
        ...
        await Promise.all(entry.updates.map((item) => api.patch(`/payments-clone-team-b/${encodeURIComponent(item.rowId)}`, item.beforePatch)));
        ...
        await api.post("/payments-clone-team-b/reorder", { items: entry.beforeOrder });
        ...
        const res = await api.post("/payments-clone-team-b", entry.row);
        ...
        await api.delete(`/payments-clone-team-b/${encodeURIComponent(currentRowId)}`);
      }
  ...
  const redoLastAction = async () => {
    ...
    try {
      if (entry.type === "updateMany") {
        ...
        await api.post("/payments-clone-team-b/reorder", { items: entry.afterOrder });
      } else if (entry.type === "deleteRow") {
        const currentRowId = getRowId(entry.row);
        setItemsImmediate(previousItems.filter((item) => String(getRowId(item)) !== String(currentRowId)));
        await api.delete(`/payments-clone-team-b/${encodeURIComponent(currentRowId)}`);
      } else if (entry.type === "addRow") {
        setItemsImmediate(insertRowByOrder(previousItems, entry.row));
        const res = await api.post("/payments-clone-team-b", entry.row);
  ...
  async function loadRows({ initial = false, silent = false } = {}) {
    try {
      ...
      const res = await api.get( `/payments-clone-team-b?page=1&limit=${fetchLimit}&month=all&year=all` );
  ...
  async function addRow(referenceRow = null) {
    try {
      ...
      const res = await api.post("/payments-clone-team-b", newRow);
      ...
      await api.post("/payments-clone-team-b/reorder", { items: buildReorderPayload(normalizedRows) });
  ...
  async function deleteRow(row) {
    ...
    mutationInFlightRef.current = true;
    try {
      await api.delete(`/payments-clone-team-b/${encodeURIComponent(rowId)}`);
      rememberHistoryEntry({
        type: "deleteRow",
        label: "Delete Row",
        row: cloneRow(row),
