...
import PaymentLast24HoursHistoryTeamBPanel from "./PaymentLast24HoursHistoryTeamBPanel.jsx";
...
export default function PaymentChangeRequestsTeamBPanel({
  open,
  onClose,
  paymentCloneId,
  rowData,
  ...
  const [last24HistoryOpen, setLast24HistoryOpen] = useState(false); // Used to toggle the inner component if needed
  ...
  async function loadLogs() {
    ...
    try {
      setLoading(true);
      // Ensure API calls point to Team B
      const res = await api.get("/payment-change-requests-team-b/logs", {
        params: { paymentCloneTeamBId: paymentCloneId },
      });
  ...
  return (
    <>
      <div style={styles.overlay} onClick={onClose}>
        <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
          <div style={styles.header}>
            <div>
              <h3 style={styles.title}>Team B Row Audit</h3>
            ...
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              <button type="button" style={styles.btn} onClick={() => setLast24HistoryOpen(true)}>
                Last 24 Hours
              </button>
      ...
      <PaymentLast24HoursHistoryTeamBPanel
